...
if ($id <= 0 || $nombres === '' || $nombre_empresa === '') {
    ...
    if (!$error_imagen) {
        if ($actualizar_imagen) {
            $stmt = $conn->prepare("UPDATE proveedores SET nombres=?, nombre_empresa=?, direccion=?, ciudad=?, pais=?, email=?, telefono=?, ruta=? WHERE id=?");
            $stmt->bind_param('ssssssssi', $nombres, $nombre_empresa, $direccion, $ciudad, $pais, $email, $telefono, $ruta_imagen, $id);
        } else {
            $stmt = $conn->prepare("UPDATE proveedores SET nombres=?, nombre_empresa=?, direccion=?, ciudad=?, pais=?, email=?, telefono=? WHERE id=?");
            $stmt->bind_param('sssssssi', $nombres, $nombre_empresa, $direccion, $ciudad, $pais, $email, $telefono, $id);
        }
